...
# instance, where it will clone the tag set as DEPLOY_TAG env var and set up
# whatever is needed to start the backend server

# run it on a remote server with this command structure:
# ssh ubuntu@<aws-hostname> DEPLOY_TAG="..." ROBOFLOW_API_KEY="..." ... 'bash -s' < deploy_ec2.sh
...
# clone new version
git clone --depth 1 --branch $DEPLOY_TAG https://github.com/RecycleRightCSE403/RecycleRight.git

cd RecycleRight/backend/
python3 -m venv venv
